...
func GenerateRulesAuto(name string, sources Deps, library bool) []RuleResult {
	...
	for _, key := range keys {
		modules = append(modules, sources[key])
	}
	modules.Sort()
	srcSet := SourceSet{
		name:     name,
...
func ruleConfigs(r *rule.Rule) []KeyValue {
	...
	for _, c := range r.Comments() {
		match := rex.FindStringSubmatch(c)
		if len(match) == 3 {
			kvs = append(kvs, KeyValue{match[1], match[2]})
		}
	}
	return kvs
}
...
func ruleConfigOr(r *rule.Rule, key string, def string) string {
	if value, exists := ruleConfig(r, key); exists {
		return value
	} else {
		return def
	}
...
func slug(name string) string {
	...
	if len(match) != 2 {
		log.Fatal("Library name " + name + " couldn't be parsed.'")
	}
